...
**A second run — this time to ship.** ios-genesis then built *Cadenza*, an AirPods-motion jump-rope counter, as a real app headed for the App Store rather than a throwaway scratch project. One session took it from idea interview to a merged PR with a **168-test suite** (independently re-run green), **six** visually-verified UI states across a **six-task graph**, HealthKit workout write-back, and a release checklist — across all **nine** phases in roughly a workday of human review time. The honest headline: the pipeline self-caught four minor test issues and passed its own visual and code review clean, while the four issues that *actually mattered* — two on-device jump-detection bugs and two first-run UX gaps — were caught by a human at the checkpoints. That isn't the pipeline failing; it's the two things a simulator structurally can't do (feel a real sensor, judge a first-run experience), and exactly what the human checkpoints — and the v0.4.0 roadmap — exist for. Full story: [the blog](https://github.com/giorgia/ios-genesis-blog).
