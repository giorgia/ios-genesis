...
- **Fanned-out phases** (developer, visual_verification, test_engineer on multi-task graphs): at each wave-end, run `git diff --name-only <previous wip commit>` in `target_project_path` and attribute each path to a task by matching against that task's `owned_files` prefixes. Paths matching no task's prefix are unattributable — add an `open_risks` entry (Violations section). **During test_engineer fan-out waves**, attribution uses the assigned test directories (each task's `owned_files` in the test dispatch, e.g. `<AppName>Tests/Tasks/<task_id>/`), not the graph's source `owned_files`; `project.yml` changes are attributable only to the registration pre-step.
