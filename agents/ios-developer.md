...
3. Open a PR via `gh pr create`, with a title summarizing the change and a body built from `pr_description_context` (reference `docs/architecture.md` and `docs/design.md` if they exist). If `pr_description_context` contains any `Closes #N` trailer lines, reproduce them **verbatim** on their own lines at the end of the PR body — do not paraphrase or summarize them, since GitHub only auto-closes the referenced issues when the exact `Closes #N` keyword appears in the PR description.
